from cartin_bank_account import (
    SavingsAccount,
    StudentAccount,
    BusinessAccount
)
...
def load_accounts():
    ...
    for line in lines:
        ...
        if line.startswith("Account Number:"):
            ...
            goal_text = (
                line
                .replace("Savings Goal:", "")
                .strip()
            )

            try:
                current["savings_goal"] = float(goal_text)
            except ValueError:
                current["savings_goal"] = 0.0
            ...
            if (
                ...
                savings_goal = current.get("savings_goal", 0.0)
                if current["account_type"] == "Savings Account":
                    account = SavingsAccount(current["account_number"], current["account_name"], current["pin"], current["balance"], savings_goal)
                elif current["account_type"] == "Student Account":
                    account = StudentAccount(current["account_number"], current["account_name"], current["pin"], current["balance"], savings_goal)
                else:
                    account = BusinessAccount(current["account_number"], current["account_name"], current["pin"], current["balance"], savings_goal)
    ...
    with open(
        ...
        for saved_account in accounts:
            ...
            if (
                ...
                saved_account._savings_goal = (
                    account.get_savings_goal()
                )
            ...
            file.write(
                f"Savings Goal: "
                f"{saved_account.get_savings_goal():.2f}\n"
            )
